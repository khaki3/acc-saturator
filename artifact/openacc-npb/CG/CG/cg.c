						...
						r[0:na+2])
{
  ...
  //---------------------------------------------------------------------
  int na_gangs = NA+1;
  #pragma acc parallel loop num_gangs((na_gangs+127)/128) vector_length(128) 
  for (i = 0; i < NA+1; i++) {
    x[i] = 1.0;
  }

  end = lastcol - firstcol + 1;
  #pragma acc parallel loop num_gangs((end+127)/128) vector_length(128) 
  for (j = 0; j < end; j++) {
    q[j] = 0.0;
    z[j] = 0.0;
    r[j] = 0.0;
						 ...
						 q[0:na+2],r[0:na+2])
{
  ...
  //---------------------------------------------------------------------
  #pragma acc parallel loop num_gangs((naa+127)/128) vector_length(128) independent
  for (j = 0; j < naa; j++) {
    q[j] = 0.0;
    z[j] = 0.0;
    r[j] = x[j];
...
//#pragma acc kernels loop private(cgit,j,k)
  ...
  //#pragma acc update host(p[0:NA+2])
	...
	*/
    ...
    //---------------------------------------------------------------------
    rho = 0.0;
	#pragma acc parallel loop num_gangs((end+1023)/1024) vector_length(1024) independent
    for (j = 0; j < end; j++) {
      z[j] = z[j] + alpha*p[j];
      r[j] = r[j] - alpha*q[j];
    }
    ...
    // p = r + beta*p
    //---------------------------------------------------------------------
	#pragma acc parallel loop num_gangs((end+127)/128) vector_length(128) independent
    for (j = 0; j < end; j++) {
      p[j] = r[j] + beta*p[j];
    } 
