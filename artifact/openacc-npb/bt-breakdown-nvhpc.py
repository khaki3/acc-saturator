...
log=np.array(extract_log("bt-breakdown-nvhpc.log")).transpose()
...
ax.set_yticks([0.75, 1.00, 1.25, 1.50, 1.75, 2.00, 2.25, 2.50], [0.75, "1.00", 1.25, "1.50", 1.75, "2.00", 2.25, "2.50"])
...
ax.zorder=2
ax2.zorder=1
ax2.fill_between(np.append(log[0],[log[0][-1]+1,log[0][-1]+2]), np.append(log[4],[100, 100]), 0, where= np.append(log[4],[100, 100]) > 0, facecolor='#B3E680', alpha=0.5, interpolate=True)
#ax2.plot(np.append(log[0],[log[0][-1]+1,log[0][-1]+2]), np.append(log[4],[100, 100]), clip_on=False, linewidth=4, linestyle='--', dashed_pattern=2.8, dashes=(4.1, 1))
ax2.set_ylim([0, 100])
...
plt.tight_layout()
plt.savefig('bt-breakdown-nvhpc.pdf', bbox_inches='tight')
